...
object fp3 {
  ...
  def removeDupes1 [X] (xs:List[X]) : List[X] = {
    xs match{
        case Nil => Nil
        case y::Nil => y::Nil
        case y1::y2::ys if y1!=y2 => y1::removeDupes1(y2::ys)
        case _::ys => removeDupes1(ys)
    }
  }
  ...
  // - removeDupes2 (List (1,1,2,3,3,3,4,4,5,6,7,7,8,9,2,2,2,9)) == List ((2,1),(1,2),(3,3),(2,4),(1,5),(1,6),(2,7),(1,8),(1,9),(3,2),(1,9))
    def aux[X] (accumulator:Int, x:X, xs:List[X]) : List[(Int,X)] = {
        xs match{
        case Nil => (accumulator,x)::Nil
        case y::ys if x!=y => (accumulator,x)::aux(1,y,ys)
        case _::ys => aux(accumulator+1,x,ys)
    }
    }
  ...
  def allDistinct (xs : List[Int]) : Boolean = {
    xs match{
        case Nil => true
        case y::ys => !member(y,ys)&&allDistinct(ys)
    }
  }
}
