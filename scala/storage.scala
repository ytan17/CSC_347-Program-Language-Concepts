...
object storage {
  ...
  val getAndSet : Int => (() => Int, Int => Unit) = {
    ...
    (n : Int) =>{
    	var getSet : Int = n
    	val get : () => Int = () => getSet
    	val set : Int => Unit = (x : Int) => (getSet = x)
    	(get,set)
    }
  ...
  val getAndSetSpy : () => (() => Int, Int => (() => Int, Int => Unit)) = {
    // TODO: Complete the definition.
    () => {
		var count = 0  //use var instead of val which can do the reassignment
		val getAndSet2 : Int => (() => Int, Int => Unit) = {
			(n : Int) =>{
		    	var getSet : Int = n
		    	val get : () => Int = () => getSet
		    	val set : Int => Unit = {(x : Int) => {count += 1; getSet = x}}
		    	(get,set)
		    }
		}
		val spy : () => Int = () => count
